    ...
    if play_mode == "play":
        ...
        if learn_mode == "fixed":
            ...
            if load_model == "y":
                ...
                print("Model loaded.")
                # Loaded q_table, learn_mode = "fixed"
                game = QGame(q_table=defaultdict(
                    lambda: [0, 0], q_table), epsilon=0.05)
            else:
            ...
            if load_model == "y":
                with open("q_table_random.pkl", "rb") as f:
                    q_table = pickle.load(f)
                print("Model loaded.")
                # Loaded q_table, learn_mode = "random"
                game = QGame(q_table=defaultdict(
                    lambda: [0, 0], q_table), epsilon=0.05, learn_mode="random")
            else:
